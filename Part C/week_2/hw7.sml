...
(* CHANGE: Add a case for Shift expressions *)
	  | Shift(dx,dy,e1) => case eval_prog(e1,env) of (*evaluate subexpression*)
	  	NoPoints => NoPoints | (*NoPoints remain NoPoints*)
	  	Point(x,y) => Point(x + dx, y + dy) | (*Point shifts by dx & dy*)
	  	Line(m,b) => Line(m, b + dy - m * dx) | (*slope unchanged, intercept changed*)
	  	VerticalLine(x) => VerticalLine(x + dx) | (*dy irrelevant in vertical line*)
	  	LineSegment(x1, y1, x2, y2) => (*endpoints of line segment shifted*)
	  		LineSegment(x1 + dx, y1 + dy, x2 + dx, y2 + dy) |
	  	_ => e (*to prevent non exhaustive search error*)

(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog e = 

	case e of LineSegment (x1,y1,x2,y2) => (*replace linesegment with close ends with point*)
			if real_close(x1,x2) andalso real_close(y1,y2) then Point(x1,y1)
		...
		(*preprocess subexpressions recursively in other cases*)
